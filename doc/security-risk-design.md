...
如果 OSV 返回的 `fixed` 是提交 SHA，系统会优先尝试找到“最早包含该修复提交的稳定 release”；如果没有稳定 release，再回退到 prerelease、tag，最后保留原值展示。
...
1. 仪表盘和组件列表展示组件级风险状态。
2. 漏洞检查页面展示漏洞编号、摘要、严重性、建议升级版本和证据链接。
...
- 风险记录必须和具体版本绑定。
- 同一组件只保留当前版本最近一次风险明细和最新摘要，重复检查会覆盖旧明细，避免历史漏洞数据堆积。
